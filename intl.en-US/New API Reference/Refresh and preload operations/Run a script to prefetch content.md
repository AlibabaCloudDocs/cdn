# Run a script to prefetch content

Alibaba Cloud Content Delivery Network \(CDN\) provides scripts that can be used to automatically refresh and prefetch content such as files and directories from origin servers. This topic describes the features and usage notes of scripts. The following example demonstrates how to use a script to automatically refresh and prefetch content. A Windows operating system is used in the example.

## Overview

Scripts can be used to automatically refresh or prefetch content from origin servers in batches. Compared with manual operations, scripts greatly simplify the process.

After you specify a file that contains URLs to be refreshed or prefetched, the script splits the file based on the number of concurrent refresh or prefetch tasks. The URLs are then refreshed or prefetched in batches. A script automatically detects whether a refresh or prefetch task is completed. The next refresh or prefetch task does not start until the current one ends.
...
-   Whether the refresh and prefetch tasks run as expected must be manually checked, which consumes a large amount of human resources and time.

## Usage notes

Before you run a script to automatically refresh or prefetch content, make sure that the following environment is deployed on the on-premises machine:
...
-   Windows and Linux operating systems must use Python 3 and have the logging module installed.
...
After Python is installed, you can run the python $scripte -h command in Windows Command Prompt.
...
In Windows Command Prompt, run the python Refresh.py -i your AccessKey ID -k your AccessKey secret -r the file names -t clear command. Example:

...
-   Remote authentication: You can specify a self-managed authentication server. After CDN nodes receive client requests, they redirect the requests to the specified authentication server.
...
-   Examples 2: The request fails the authentication. The CDN node returns the 404 HTTP status code to the client.
...
For example, if you select the value $http\_host, host=$http\_host is added to the URLs of requests before they are redirected to the authentication server. In this case, host indicates the Host field in the request header. For more information about the variables, see [\#d6e283](#d6e283). |
    |**Passes Authentication**|The HTTP status code returned by the authentication server if a request passes the authentication. The HTTP status code indicates the authentication result.If you set the HTTP status code to 200, the authentication server returns the 200 HTTP status code to CDN nodes for client requests that pass the authentication. If the HTTP status code returned by the authentication server does not indicate whether the request passes or fails the authentication, the authentication on the request times out. |
    |**Fails Authentication**|The HTTP status code returned by the authentication server if a request fails the authentication. The HTTP status code indicates the authentication result.If you set the HTTP status code to 403, the authentication server returns the 403 HTTP status code to CDN nodes for client requests that fail the authentication. If the HTTP status code returned by the authentication server does not indicate whether the request passes or fails the authentication, the authentication on the request times out. |
    |**Custom HTTP Status Code**|The HTTP status code returned from the CDN node to the client after the CDN node receives an HTTP status code, which indicates that the request fails the authentication, from the authentication server.If you set the HTTP status code to 403, the CDN node returns the 403 HTTP status code to clients for requests that fail the authentication. |
    |**Timeout**|The timeout period starts when the CDN node redirects the request to the authentication server and ends when the CDN node receives the authentication result from the authentication server. The timeout period is measured in milliseconds. You can set the timeout period to at most 3,000 milliseconds.|
    |**Action After Timeout**|The action that the CDN node performs on a request if the authentication on the request times out. **Allow** and **Reject** are supported:    -   **Allow**: If the authentication on the request times out, the CDN node returns the requested resources to the client.
...
|$http\_content\_type|Indicates the value of the Content\_Type header field.|
|$http\_x\_forward\_for|Indicates the value of the X\_Forward\_For header field.|

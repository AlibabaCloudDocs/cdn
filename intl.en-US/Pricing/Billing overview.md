...
Alibaba Cloud CDN is billed for [basic services](#table_wbd_iye_txc) and [value-added services](#table_dxt_iy7_zj4). For more information, see [CDN Pricing](https://www.alibabacloud.com/product/cdn/pricing).

|Billable item|Billing rule|Metering method|Billing method|
|-------------|------------|---------------|--------------|
|Data transfer\(default\)

|You are charged based on tiered pricing in each billable region. The billing cycle is one calendar month. Within a billing cycle, the amount of data transfer that exceeds the quota in the current pricing tier is billed based on the next pricing tier.|Pay-by-data-transfer|Pay-as-you-go: The bill of a billing cycle is issued about 3 hours after the billing cycle ends. The time when bills are issued is determined by the system.|
...
|Peak bandwidth|You are charged based on the daily peak bandwidth values in each region. Unit: Mbit/s. **Note:** Beginning January 8, 2021, basic services of Alibaba Cloud CDN use the pay-by-data-transfer metering method by default. To change to the pay-by-bandwidth metering method, [submit a ticket](https://workorder-intl.console.aliyun.com/?spm=5176.2020520001.aliyun_topbar.18.dbd44bd3e4f845#/ticket/createIndex) or contact your sales manager.
...
|Billable item|Billing rule|Metering method|Billing method|
|-------------|------------|---------------|--------------|
|Web Application Firewall \(WAF\)|Resource plans for WAF: You can make a one-off payment for resource plans. These resource plans use the subscription billing method and are valid for one year. After you complete the payment for a resource plan, it immediately takes effect. The amount of resources that exceeds the resource plan is billed on a pay-as-you-go basis.|Pay-as-you-go: You are charged based on the number of requests.|Subscription|
...
|Real-time log delivery|To enable the real-time log delivery service, log on to the [Alibaba Cloud CDN console](https://cdn.console.aliyun.com) and choose **Logs** \> **Real-time Logs** in the left-side navigation pane. You are charged for log entries that are delivered to a specified location.|Pay-as-you-go: You are charged based on the number of log entries delivered to a specified location.|Pay-as-you-go|
|Content moderation|Resource plans for content moderation: You can make a one-off payment for resource plans. These resource plans use the subscription billing method and are valid for one year. After you complete the payment for a resource plan, it immediately takes effect. The number of images that exceeds the resource plan is billed on a pay-as-you-go basis.|Subscription|
...
Alibaba Cloud CDN allows you to query bills by day or by month. For more information, see [Query bills](/intl.en-US/Pricing/Usage and bills/Query bills.md).

...
|DomainNames|String|是|example.com|加速域名，多个使用英文逗号（,）分隔。 |
...
|ttl：缓存时间，单位为秒。
 ...
 file\_type：文件类型，多个使用英文逗号（,）分隔，例如TXT,JPG。 |
...
|hashkey\_args：保留参数的列表，多个使用英文逗号（,）分隔，最多支持填写10个。
...
|（必填）file\_type：多个使用英文逗号（,）分隔，例如：txt,jpg。

 （必填）code\_string：状态码，多个使用英文逗号（,）分隔。例如：302=0,301=0,4xx=2。 |
...
|（必填）switch：开关，支持on/off，开启功能。一旦开启后，如需关闭请取消勾选，不要删除配置，否则不生效。
...
|（必填）enable：开关，支持on或off，支持TS、MP3文件格式，FLV和MP4使用拖拽功能实现。

 （必填）ali\_video\_preview\_argument：自定义试看参数名，试看参数值的单位必须是秒。 |
|default\_ttl\_code：状态码缓存时间

|（必填）default\_ttl\_code：状态码默认缓存时间，单位为秒，例如：4xx=3,200=3600,5xx=1，多个使用英文逗号（,）分隔。 |
...
|（必填）enabled：开关，支持l2或off。

 bucketname：Amazon S3 bucket名称。

 （必填）accesskey：AWS AccessKey。

 （必填）secretkey：AWS SecretKey。

 （必填）region：Amazon S3存储区域。 |
|origin\_request\_header：回源HTTP请求头

|（必填）header\_operation\_type：请求头操作，支持add、delete、modify和rewrite，add：添加，delete：删除，modify：变更，rewrite：替换。

 （必填）header\_name：请求头名称。
...
|（必填）header\_operation\_type：响应头操作，支持add、delete、modify和rewrite，add：添加，delete：删除，modify：变更，rewrite：替换。

 （必填）header\_name：响应头名称。
